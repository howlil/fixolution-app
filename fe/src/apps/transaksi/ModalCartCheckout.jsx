      ...
      <section className="p-6 bg-white rounded-xl w-[34rem]">
        ...
        <div className="mt-4 flex w-full justify-between items-center gap-2">
          <img
            ...
            src={`${import.meta.env.VITE_API_BASE_URL}/fotoSukuCadang/${latestItem?.foto}`}
            alt={latestItem?.nama}
          />
          <div>
            <div>
              <h2 className="text-lg font-semibold">{latestItem?.nama}</h2>
              <p className="text-sm text-gray-500">Jumlah: {latestItem?.quantity}</p>
              <p className="text-gray-500 text-md mt-4">Total: Rp. {latestItem?.total}</p>
            </div>
            <div className="mt-6 w-full  grid grid-cols-2 gap-4 justify-between">
              ...
              <Button
                ...
                onClick={()=>navigate(`/checkout/${latestItem?.keranjang_item_id}`)}
              >
                Checkout
              </Button>
            </div>
